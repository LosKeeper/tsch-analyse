...
tschOrchestraExperimentLaunchCoordSender1Log(){
    ...
    local idExp = `iotlab-experiment submit -n tsch-orchestra-1send-1coord-log -d 5 -l strasbourg,m3,1,coordinator.iotlab -l strasbourg,m3,2,sender.iotlab | grep "id" | cut -d' ' -f6 | cut -d' ' -f1`
    echo $idExp
